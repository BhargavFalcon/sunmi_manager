...
class PusherChannelsFlutterPlugin : FlutterPlugin, MethodCallHandler, ActivityAware,
    ...
    private fun init(
        ...
        try {
            ...
            val customHost = call.argument<String>("host")
            if (customHost != null && customHost.isNotEmpty()) {
                options.setHost(customHost)
                (call.argument<Int>("wsPort") ?: (call.argument<Long>("wsPort")?.toInt()))?.let { options.setWsPort(it) }
                (call.argument<Int>("wssPort") ?: (call.argument<Long>("wssPort")?.toInt()))?.let { options.setWssPort(it) }
