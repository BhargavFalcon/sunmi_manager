...
public class SwiftPusherChannelsFlutterPlugin: NSObject, FlutterPlugin, PusherDelegate, Authorizer {
  ...
  func initChannels(call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    if useTLS {
      port = 443
      if let p = args["wssPort"] as? Int { port = p }
      else if let p = (args["wssPort"] as? NSNumber)?.intValue { port = p }
    } else {
      port = 80
      if let p = args["wsPort"] as? Int { port = p }
      else if let p = (args["wsPort"] as? NSNumber)?.intValue { port = p }
    }
